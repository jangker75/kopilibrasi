<?php namespace App\Http\Controllers;
	...
	class AdminPenjualanController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form[] = ['label'=>'Total Price','name'=>'total_price','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
	    ...
	    */
		...
		{
			// $data["query"] = SalesTransaction::with('salesdetail')->get();
			$data["data"] = DB::table('t_sales_header as h')
								->selectRaw("h.date, sum(d.base_price) as total_price, sum(d.sale_price) as total_sale, sum(d.discount) as total_discount, sum(d.qty) as total_item")
								->join('t_sales_detail as d', 'h.id','=','d.sales_id')
								->groupBy("date")->get();
			$data["dataTable"] = [];
			return view("backend.penjualan.index", $data);
		}
		public function getDetail($date)
		{
			$data["data"] = SalesTransaction::with('salesdetail')
							->where("date", $date)->get();
			dd($data);
			return view("backend.penjualan.detail", $data);
		}
